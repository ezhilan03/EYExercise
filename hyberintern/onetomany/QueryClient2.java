...
public class QueryClient2 {
	...
	public void query2()
	{
		...
		//or
//		criteria.add(Property.forName("ename").eq("somu"));
	...
	public void query3()
	{
		...
		Address add=(Address)criteria.uniqueResult();
...
//		}
	...
	{
		Session session=HibernateUtility.currentSession();
		int f[]={new Integer(20),new Integer(2000)};
		Criteria criteria=session.createCriteria(Employee.class);
		criteria.add(Restrictions.between("esal",new Integer(1001),new Integer(2000)));
		//or
//		criteria.add(Property.forName("esal").in(f));
	...
	public void query6()
	{
		...
		criteria.
			...
			.add(Restrictions.gt("esal", new Integer(20)));
		display(criteria);
	}
	...
	/*
	 * The detached criteria allows you to create the query without Session . ... They are most useful
	 * for join conditions, subselects, and to query outside the current session. Another common use
	 * is for code reuse. Many developers declare them as static queries and execute them using the
	 * underlying session from different DAO .
	 */
	public void query8()
	{
		Session session=HibernateUtility.currentSession();
